...
Методы, которые считаются безопасными для **CORS**: `GET`, `HEAD`, `POST`.

Для проверки на соответствие одному источнику существует алгоритм [CORS check](https://fetch.spec.whatwg.org/#concept-cors-check), в который передаются запрос и ответ.
