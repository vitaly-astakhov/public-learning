...
- [Cookie Attributes](#cookie-attributes)
  ...
  - [SameSite](#samesite)
  - [Prefixes](#prefixes)
    - [The "\_\_Secure-" Prefix](#the-__secure--prefix)
    - [The "\_\_Host-" Prefix](#the-__host--prefix)
- [API's](#apis)
...
Атрибуты срока жизни (*lifetime*) файлов cookie, такие как `Expires` и `Max-Age` выделяют файлы cookie в две категории:

- Постоянные файлы cookie (*permanent *cookies*) удаляются в день, указанный в атрибуте `Expires`, или по истечении периода, указанного в атрибуте `Max-Age`.
- Сессионные файлы cookie (*session cookies*) – файлы без атрибутов `Max-Age` или `Expires` – удаляются по окончании текущего сеанса. Браузер определяет, когда заканчивается "текущий сеанс", и некоторые браузеры используют функцию восстановления сеанса при перезапуске. Это может привести к тому, что файлы cookie сеанса будут храниться бесконечно долго.
...
При работе в HTML файлами куки можно управлять через несколькими способами

- [`document.cookie`](https://html.spec.whatwg.org/multipage/dom.html#dom-document-cookie)
- [`CookieStore` Interface](https://wicg.github.io/cookie-store/#CookieStore)

Узнать, разрешена ли передача файлов куки, можно через аттрибут [`window.navigator.cookieEnabled`](https://html.spec.whatwg.org/multipage/system-state.html#dom-navigator-cookieenabled)
