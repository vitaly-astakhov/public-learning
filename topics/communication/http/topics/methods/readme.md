...
Метод запроса (*request method*) - часть request семантики и указывает на цель, с которой клиент сделал этот запрос, и что ожидается клиентом в качестве успешного результата.

Метод запроса (*request method*) вызывает действие, которое должно быть применено к целевому ресурсу ([*target resource*](https://www.rfc-editor.org/rfc/rfc9110.html#target.resource)).

Токен, который присваивается методу является чувствительным к регистру (*case-sensitive*) [^4], поскольку он может использоваться в качестве шлюза для объектно-ориентированных систем с чувствительными к регистру именами методов.
...
[^4]: <https://www.rfc-editor.org/rfc/rfc9110.html#section-9.1-5>
